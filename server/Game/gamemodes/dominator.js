    ...
    o.on("dead", () => {
      ...
      if (team === TEAM_ENEMIES) {
        ...
        global.gameManager.socketManager.broadcast(
          `A dominator is now controlled by ${teamName}!`,
        );

        if (!this.teamcounts[newTeam]) {
          this.teamcounts[newTeam] = 0;
        }
        this.teamcounts[newTeam]++;

        if (
          newTeam !== TEAM_ENEMIES &&
          this.teamcounts[newTeam] >= this.neededToWin &&
          !this.gameWon
        ) {
          this.gameWon = true;
          setTimeout(() => {
        ...
        global.gameManager.socketManager.broadcast(
          "A dominator is being contested!",
        );
      }
