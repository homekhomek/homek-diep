...
let commands = [
      ...
      let lines = [
        ...
        ...commands
          ...
          .map((c) => {
            ...
            let commandText = cmdData
              .map((e) => e.map((name) => name).join(` or ${prefix} `))
              .join(" ");
  ...
  },
  {
    ...
    run: ({ socket, args }) => {
      ...
      const leaderboards = ["default", "players", "bosses", "global"];
      const choice = args[0];
  ...
  },
  {
    ...
    run: ({ socket }) => {
      ...
      socket.talk(
        "m",
        3_000,
        `In-game chat ${socket.status.disablechat ? "enabled" : "disabled"}`,
      );
    },
  },
  {
    ...
    run: ({ socket, args, gameManager }) => {
      ...
      if (!args[0]) sendAvailableArenaMessage();
      else {
        switch (args[0]) {
          case "size":
            if (args[1] === "dynamic") {
              if (!Config.sandbox)
                return socket.talk(
                  "m",
                  3_000,
                  "This command is only available on sandbox.",
                );
              ...
              if (args[1] % 2 === 0 && args[2] % 2 === 0) {
                if (Config.sandbox)
                  gameManager.room.settings.sandbox.do_not_change_arena_size = true;
          ...
          case "close":
            util.warn(
              `${socket.player.body.name === "" ? `An unnamed player (ip: ${socket.ip})` : socket.player.body.name} has closed the arena.`,
            );
  ...
  },
  {
    ...
    run: ({ socket, args, gameManager }) => {
      ...
      if (command === "reloaddefs" || command === "redefs") {
        ...
        let sinceLastReload =
          time - global.reloadDefinitionsInfo.lastReloadTime;
        if (sinceLastReload < 5000) {
          socket.talk(
            "m",
            Config.popup_message_duration,
            `Wait ${Math.floor((5000 - sinceLastReload) / 100) / 10} seconds and try again.`,
          );
          return;
        }
        ...
        // Log it.
        util.warn(
          `[IMPORTANT] Definitions are going to be reloaded on server ${gameManager.gamemode} (${gameManager.webProperties.id})!`,
        );

        // Purge all cache entries of every file in definitions
        for (let file in require.cache) {
          if (!file.includes("definitions") || file.includes(__filename))
            continue;
        ...
        setTimeout(() => {
          ...
          gameManager.clients.forEach((socket) => {
            ...
            socket.status.selectedLeaderboard2 =
              socket.status.selectedLeaderboard;
          ...
          // Log it again.
          util.warn(
            `[IMPORTANT] Definitions are successfully reloaded on server ${gameManager.gamemode} (${gameManager.webProperties.id})!`,
          );
...
function runCommand(socket, message, gameManager) {
  ...
  let command = commands.find((command) =>
    command.command.includes(commandName),
  );
  if (command) {
...
global.addChatCommand = function (command) {
  if (!command.command || !command.run) {
    throw new Error(
      "Invalid command format. A command must have at least a 'command' and a 'run' property.",
    );
  }
  if (!Array.isArray(command.command)) {
    throw new Error(
      "Invalid command format. The 'command' property must be an array of strings.",
    );
  }
  if (
    commands.find((c) => c.command.some((cmd) => command.command.includes(cmd)))
  ) {
...
module.exports = ({ Events }) => {
  Events.on(
    "chatMessage",
    ({ socket, message, preventDefault, gameManager }) => {
