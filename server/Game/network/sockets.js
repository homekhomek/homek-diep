...
class socketManager {
  ...
  broadcastRoom() {
    for (let i = 0; i < this.clients.length; i++) {
      this.clients[i].talk(
        ...
        JSON.stringify(
          global.gameManager.room.setup.map((x) =>
            x.map((t) => {
              return {
                color: t.color,
                image: t.image ?? false,
  ...
  close(socket) {
    ...
    if (index != -1) {
      ...
      if (player.body != null) {
        ...
        if (socket.status.transferred) {
          ...
          let timeout = setTimeout(() => {
            ...
            util.remove(
              this.disconnections,
              this.disconnections.indexOf(disconnection),
            );
          }, 60000);
          let disconnection = {
            body: player.body,
      ...
      util.log(
        "[INFO] " +
          (player.body
            ? `User ${player.body.name == "" ? "A unnamed player" : player.body.name}`
    ...
    // Free the view
    util.remove(
      global.gameManager.views,
      global.gameManager.views.indexOf(socket.view),
    );
    // Remove the socket
    ...
    util.log(
      "[INFO] The connection has closed. Views: " +
        global.gameManager.views.length +
        ". Clients: " +
        this.clients.length +
        ".",
    );
      ...
      case "s":
        {
          ...
          if (this.clients.length > global.gameManager.webProperties.maxPlayers)
            return (
              socket.talk(
                "message",
                "This server is full, please rejoin later.",
              ),
          ...
          let permB = permBans.find((bannedIP) => bannedIP.ip === socket.ip);
          if (permB) {
          ...
          if (global.gameManager.arenaClosed) {
            if (needsRoom) {
              socket.talk(
                "message",
                "Arena closed. Try again in a few seconds.",
              );
      ...
      case "C":
        {
          ...
          if (!socket.player.body.eastereggs.braindamage)
      ...
      case "U":
        {
          ...
          if (
            ...
            if (!upgrade.isDailyUpgrade) {
              // Atleast allow the daily upgrade request, else get out.
      ...
      case "L":
        {
          ...
          if (
            player.body.skill.level < Config.level_cap_cheat ||
      ...
      case "H":
        {
          ...
          if (body.underControl) {
            let relinquishedControlMessage = Config.domination
              ? "dominator"
              : Config.mothership
                ? "mothership"
                : "special tank";
            if (Config.domination || Config.mothership) {
              player.body.sendMessage(
                `You have relinquished control of the ${relinquishedControlMessage}.`,
              );
          ...
          if (Config.mothership) {
            ...
            if (!motherships.length) {
              player.body.sendMessage(
                "There are no motherships available that are on your team or already controlled by an player.",
              );
              return 1;
            }
            ...
            if (!dominators.length) {
              player.body.sendMessage(
                "There are no dominators available that are on your team or already controlled by an player.",
              );
              return 1;
            }
      ...
      case "DTA":
        {
          if (
            ...
            player.body.skill.level >=
              Config.tier_multiplier * Config.daily_tank.tier &&
            Config.daily_tank.ads.enabled &&
            !socket.status.daily_tank_watched_ad
          ) {
            let chosenAd = ran.choose(Config.daily_tank.ads.source);
            let isImage =
              chosenAd.file.endsWith(".png") ||
              chosenAd.file.endsWith(".jpg") ||
              chosenAd.file.endsWith(".jpeg");
            socket.talk(
              "DTA",
              JSON.stringify({
                src: chosenAd.file,
                normalAdSize: chosenAd.use_regular_ad_size ?? true,
                waitTime: isImage ? chosenAd.image_wait_time : "isVideo",
              }),
            );
            if (isImage) {
              setTimeout(() => {
  ...
  spectateEntity(possible, socket) {
    ...
    for (const entry of entities.values()) {
      if (possible.includes("arenaCloser") && entry.isArenaCloser)
        entries.push(entry);
  ...
  update(gui) {
    ...
    // Update most things
    gui.fps.update(
      Math.min(1, (global.fps / global.gameManager.roomSpeed / 1000) * 30),
    );
    ...
    if (Config.daily_tank) {
      if (
        b.skill.level >= Config.tier_multiplier * Config.daily_tank.tier &&
        b.defs.includes(Config.spawn_class)
      ) {
        dailyTank = Config.daily_tank_INDEX;
      }
      gui.dailyTank.update(
        JSON.stringify([
          dailyTank,
          Config.daily_tank.ads.enabled &&
          !b.socket.status.daily_tank_watched_ad
            ? true
            : false,
        ]),
      );
  ...
  initalizePlayer(epackage, socket) {
    ...
    if (transferbodyID) {
      let bodyInfo = global.travellingPlayers.find(
        (i) => i.id === transferbodyID,
      );
      if (bodyInfo) {
        spawn = false;
        ...
        for (let i = 0; i < Config.level_cap_cheat; i++)
        ...
        util.remove(
          ...
          global.travellingPlayers.indexOf(bodyInfo),
        );
      }
    }
    if (spawn) {
      socket.player = socket.spawn(name);
      setTimeout(() => {
        // Give the entity a small time to prepare.
        ...
        if (eastereggs.braindamage) {
          ...
          let braindamageloop = setInterval(() => {
            if (socket.player.body == null)
              return clearInterval(braindamageloop);
    ...
    // Log it
    util.log(
      `[INFO] [${global.gameManager.name}] ${name == "" ? "An unnamed player" : name} has spawned into the game on team ${socket.player.body.team}! Players: ${this.players.length}`,
    );
    // Stop the timeout
    socket.timeout.stop();
  }
  ...
  getSpawnLocation(rememberedTeam, name) {
    console.log("yo: ", rememberedTeam, name);
    let player = {},
      loc = {};
    ...
    if (Config.clan_wars && name) {
      ...
      return {
        player: Config.clan_wars_ft.getPlayerInfo(name),
        loc: Config.clan_wars_ft.getSpawn(name),
      };
    }
    if (Config.mode == "tdm" || Config.tag) {
      if (name) {
        ...
        // If no tags, run the standard auto-balancer
  ...
  spawn = (socket, name) => {
    let { player, loc } = this.getSpawnLocation(socket.rememberedTeam, name);
    if (socket.player.loc && !global.spawnPoint && !Config.clan_wars)
      loc = socket.player.loc;
    // Create and bind a body for the player host
    let body;
    const filter = this.disconnections.filter(
    ...
    if (filter.length) {
      ...
      util.remove(
        body.controllers,
        body.controllers.indexOf(
          body.controllers.find((rer) => rer instanceof ioTypes.listenToPlayer),
        ),
      );
      body.become(player);
  ...
  preparePlayer(socket, player, body, doNotTakeAction = {}) {
    ...
    player.teamColor = new Color(
      !Config.random_body_colors &&
        (Config.groups ||
          Config.mode == "ffa" ||
          (Config.mode == "clan" && !Config.tag))
        ? 10
        : global.getTeamColor(body.team),
    ).compiled; // blue
  ...
  flatten(data) {
    ...
    if (data.type & 0x01) {
      ...
      if (data.type & 0x04) {
        output.push(/* 17 */ data.name, /* 18 */ data.score);
      }
    }
    ...
    for (let i = 0; i < data.turrets.length; i++)
      output.push(...this.flatten(data.turrets[i]));
    // Return it
    return output;
  }
  ...
  perspective(e, player, data) {
    if (player.body != null) {
      if (player.body.id === e.master.id) {
        ...
        player.teamColor = new Color(
          !Config.random_body_colors &&
            (Config.groups ||
              ...
              (Config.mode == "clan" && !Config.tag))
            ? 10
            : global.getTeamColor(player.body.team),
        ).compiled; // blue
        ...
        if (!e.limited && !player.body.settings.canSeeInvisible)
          data[18] = Math.round(255 * alpha);
      }
  ...
  generateMockup(index) {
    ...
    let mock;
    let find = Object.keys(Class).find(
      (o) => Class[o] && Class[o].index === index,
    );
    if (find) {
  ...
  sendMockup(index, socket) {
    for (let splittedIndex of index.toString().split("-")) {
      if (socket.status.mockupData.receivedIndexes.includes(splittedIndex))
        continue; // Do NOT continue if we have the mockup already.
      ...
      if (mockup.sendAllMockups) {
        // Send all of its upgrades if needed to prevent bugs.
        // Target the upgrades
  ...
  sendMockupUpgrades(index, socket) {
    for (let splittedIndex of index.toString().split("-")) {
      if (
        socket.status.mockupData.receivedUpgradePackIndexes.includes(
          splittedIndex,
        )
      )
        continue; // Do NOT continue if we have the mockup already.
      ...
      for (let { index } of mockup.upgrades) {
        for (let i of index.toString().split("-"))
          this.sendMockupUpgrades(i, socket);
      }
    }
  }
  ...
  eyes(socket) {
    const check = (camera, obj) => {
      ...
      return (
        Math.abs(obj.x - camera.x) < camera.fov * fov + 1.5 * obj.size + 100 &&
        Math.abs(obj.y - camera.y) <
          camera.fov * fov * 0.5625 + 1.5 * obj.size + 100
      );
    };
    ...
    let o = {
      ...
      gazeUpon: (updateCam = false) => {
        ...
        if (player.body != null) {
          ...
          if (player.body.isDead()) {
            ...
            if (player.body.store && player.body.store.dragInterval) {
              // If we are still dragging a entity, clear it and delete it.
              clearInterval(player.body.store.dragInterval);
              delete player.body.store.dragInterval;
            }
            let die = () => {
              // The only reason this exist is because of bacteria's abilities.
        ...
        if (player.body == null) {
          // if we have no body, then u dead bro.
          fovNow = 2000;
          camera.scoping = false; // No scoping bugs!
          if (socket.spectateEntity != null) {
            // If we want to spectate someone, we spectate it.
        ...
        if (
          ...
          const camFovBroad =
            camera.fov * (global.gameManager.arenaClosed ? 1.6 : 1);
        ...
        for (const entity of nearby.values()) {
          ...
          if (
            ...
            visible.push(
              this.perspective(entity, player, entity.flattenedPhoto),
            );
          }
        }
        ...
        if (updateCam) {
          socket.talk("u", true, camera.x, camera.y);
        } else {
          // Update the gui
          player.gui.update();
  ...
  deltaHandler = (() => {
    ...
    let makeLeaderboardList = (list, args) => {
      ...
      for (let i = 0; i < 10 && list.length; i++) {
        ...
        if (is === 0) break;
        let entry = list[top];
        let color = entry.leaderboardColor
          ? entry.leaderboardColor + " 0 1 0 false"
        ...
        topTen.push({
          ...
          data: [
            ...
            entry.index,
            entry.name,
            entry.leaderboardColor
              ? color
              : Config.mode == "ffa" && !Config.tag
                ? "12 0 1 0 false"
                : color,
            color,
    ...
    let minimapAll = new Delta(5, (args) => {
      ...
      for (const my of entities.values()) {
        if (
          ...
          const x = Config.blackout
            ? Math.floor(
                Math.random() * global.gameManager.room.width -
                  global.gameManager.room.width / 2,
              )
            : my.x;
          const y = Config.blackout
            ? Math.floor(
                Math.random() * global.gameManager.room.height -
                  global.gameManager.room.height / 2,
              )
            : my.y;
          all.push({
            id: my.id,
            data: [
              Config.blackout
                ...
                : my.type === "wall" || my.isMothership
                  ...
                  : 0,
              util.clamp(
                Math.floor((256 * x) / global.gameManager.room.width),
                -128,
                127,
              ),
              util.clamp(
                Math.floor((256 * y) / global.gameManager.room.height),
                -128,
                127,
              ),
              Config.blackout
                ? Config.blackout_minimap_color + " 0 1 0 false"
                : my.minimapColor
      ...
      for (const my of entities.values())
        if (
          ...
          all.push({
            ...
            data: [
              util.clamp(
                Math.floor((256 * my.x) / global.gameManager.room.width),
                -128,
                127,
              ),
              util.clamp(
                Math.floor((256 * my.y) / global.gameManager.room.height),
                -128,
                127,
              ),
              my.minimapColor
    ...
    let bossLeaderboard = new Delta(7, (args) => {
      ...
      for (const instance of entities.values()) {
        if (
          (instance.isBoss || instance.type == "miniboss") &&
          instance.settings.leaderboardable &&
          instance.settings.drawShape
        )
          list.push(instance);
    ...
    setInterval(() => {
      ...
      for (let socket of subscribers) {
        ...
        if (!socket.status.selectedLeaderboard)
          socket.status.selectedLeaderboard = "global";
        if (
          !socket.status.hasSpawned ||
        ...
        let team = socket.status.seesAllTeams
          ? minimapAllTeamsUpdate
          : minimapTeamUpdates;
      ...
      for (let socket of this.clients) {
        ...
        if (time - socket.statuslastHeartbeat > Config.max_heartbeat_interval)
  ...
  sendToServer(socket, server) {
    ...
    fetch(`${server}/api/sendPlayer`, {
      ...
      body: JSON.stringify({
        ...
        definition: socket.player.body.defs.map(
  ...
  disconnect(socket) {
    ...
    if (check) {
      ...
      util.log(
        `[INFO]: [${this.gamemode}] ${check.player.body ? check.player.body.name : "A Client"} has disconnected!`,
      );
      // Free the view
      util.remove(
        global.gameManager.views,
