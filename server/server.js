...
console.log(
  `Loader successfully loaded all files. Info: [ Created Date: ${GLOBAL.creationDate} Created Time: ${GLOBAL.creationTime} ]`,
);
...
if (Config.allow_ACAO && Config.startup_logs) {
  util.warn(
    "Access-Control-Allow-Origin is enabled, which allows any server/client to access data from the WebServer.",
  );
}
...
server = require("http").createServer((req, res) => {
  ...
  for (let server of global.servers)
    if (server.ip !== Config.host && server.ip) {
      let http = server.ip.startsWith("localhost")
        ? `http://${server.ip}`
        : `https://${server.ip}`;
      serversIP.push(http);
    }
    ...
    case selectedHeader:
      {
        ...
        res.writeHead(200, {
          "Content-Type": mimeSet[extension] || "text/html",
        });
...
function loadGameServer(
  ...
  if (!loadViaMain) {
    ...
    setTimeout(() => {
      // Space it a little out.
      if (global.launchedOnMainServer) {
        console.warn(
          "Only one server can be loaded via through the main server!\nProcess terminated.",
        );
        process.exit(1);
      }
...
global.onServerLoaded = () => {
  ...
  if (loadedServers >= global.servers.length) {
    ...
    console.log(
      "Server loaded in " +
        util.rounder(serverStartEndTime, 4) +
        " milliseconds.",
    );
