...
let getNow = () => {
  ...
  gui = {
    ...
    __s: {
      setScore: (d) => {
        d
          ? (sscore.set(d), deduction > sscore.get() && (deduction = level = 0))
          : ((levelscore = 3),
            (deduction = level = 0),
            (sscore = util.AdvancedSmoothBar(0, 2)));
      },
  ...
  if (type & 0x01) {
    ...
    if (isNew) {
      z.render = {
        ...
        lastx:
          z.x -
          global.metrics.rendergap * config.roomSpeed * (1000 / 40) * z.vx,
        lasty:
          z.y -
          global.metrics.rendergap * config.roomSpeed * (1000 / 40) * z.vy,
        lastvx: z.vx,
        lastvy: z.vy,
        lastf: z.facing,
        f: z.facing,
  ...
  gui: () => {
    ...
    if (indices.upgrades) {
      ...
      for (let i = 0, len = get.next(); i < len; i++) {
        ...
        gui.upgrades[i][2] = util.requestEntityImage(
          gui.upgrades[i][2],
          gui.color,
        );
      }
...
let incoming = async function (message, socket) {
  ...
  switch (m.shift()) {
    ...
    case "bansussy":
      {
        global.message =
          "You have been temporarily banned from the game. You will be able to rejoin after a server restart.";
      }
      break;
    case "permanentban":
      {
    ...
    case "svInfo":
      {
        ...
        if (global.showDebug)
          console.log(
            `mspt: ${global.serverStats.mspt} total entities on screen: ${global.entities.length} Player X: ${global.player.renderx.toFixed(1)} Player Y: ${global.player.rendery.toFixed(1)}`,
          );
      }
      break;
    case "gSvInfo":
    ...
    case "S":
      {
        ...
        if (sync.length < 10) {
          ...
          socket.talk(
            ...
            global.playerName,
            0,
            1 * config.game.autoLevelUp,
            global.bodyID ? global.bodyID : false,
          );
    ...
    case "u":
      {
        ...
        if (m[0] == true) {
          // Update camera only if we want to.
          let camx = m[1],
            camy = m[2];
    ...
    case "F":
      {
        ...
        global.finalKills = [
          util.AdvancedSmoothBar(0, 4),
          util.AdvancedSmoothBar(0, 5.5),
          util.AdvancedSmoothBar(0, 2.5),
          util.AdvancedSmoothBar(0, 6),
        ];
    ...
    case "DTAD":
      {
        if (global.dailyTankAd.requestInterval)
          clearInterval(global.dailyTankAd.requestInterval);
    ...
    case "SH":
      {
        let data = JSON.parse(m[0]);
        if (data.type == "camera") {
          // If the server wants to shake our camera...
        ...
        if (data.type == "gui") {
          // If the server wants to shake our GUI...
    ...
    case "t":
      {
        ...
        global.bodyID = m[1];
        if (global.serverMap[global.serverAdd])
          global.serverMap[global.serverAdd].onclick();
    ...
    case "AS":
      {
        // Activating smooth camera if needed.
    ...
    case "DS":
      {
        // Deactivate smooth camera if needed.
        if (!config.graphical.smoothcamera2)
          config.graphical.smoothcamera = false;
        ...
        for (let i = get.next(); i; i--) {
          ...
          for (let j = 0; j < count; j++) {
            ...
            const alreadyExists = existing.some(
              (msg) => msg.text === text && msg.expires === expires,
            );
            if (!alreadyExists) {
              newMessages.push({
                text,
...
global.resetSocket = () => {
  ...
  serverStart = 0;
  sscore.set(0);
  ((gui.points = 0), (gui.playerid = -1), (gui.class = ""));
  gui.root = "";
  minimap.map = {};
  minimapAllInt.elements = {};
  minimapTeamInt.elements = {};
